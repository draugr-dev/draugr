...
func (d Document) Claims(source string) []Claim {
	...
	for _, st := range d.Statements {
		...
		// A statement with no products at all still says something about the vulnerability. Kept with
		// an empty PURL, which Index treats as applying to any package, narrowed by the component that
		// declared the source, which is the whole reason that scoping exists.
...
func normalizePURL(id string) string {
	...
	// product IRI. Is returned as written, because guessing at its shape is how a match happens for
	// the wrong reason.
