...
func buildVEX(d Data) vexDocument {
	...
	for _, name := range controls {
		for _, res := range d.Run.Controls[name].Report.Results {
			...
			st := statementFor(res)
			// Keep the strongest claim of exposure when a CVE appears more than once, in two components,
			// or in two lockfiles of the same one. VEX statements are per product, and this document has
			// one product, so the instances have to resolve to a single answer rather than contradict each
			// other inside the same file.
...
//
// Defaults to a purl over the release, because the release *is* the product. VEX is asked for per
// shipped thing, not per repository. `pkg:generic/` is honest about being synthesized from the
// descriptor rather than taken from anywhere real, and config.vex.product replaces it with
...
// identifier from going stale: a VEX statement is about a *version* of a product, a not_affected
// that held in 2.3 says nothing about 2.4, so a product string with the version written into it
// silently keeps claiming the old one after the release moves on. Writing `pkg:oci/acme/api` and
// letting the release supply the version cannot drift.
//
...
func vexProductID(d Data) string {
	...
	// A descriptor naming no project has no product to identify, and `pkg:generic/@2.4.0` is not a
	// package URL. It parses, it is understood, and it matches nothing, which is worse than an
...
// `https://acme.example/products/api` would produce something that is neither the IRI given nor a
// valid anything else.
