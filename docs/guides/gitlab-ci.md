...
| **License Compliance** | a CycloneDX SBOM, from `config.sbom` | Ultimate |
...
### License Compliance needs an SBOM, not a report

GitLab reads licences out of a **CycloneDX SBOM** rather than a report of its own — the older
`license_scanning` artifact is not what populates that tab. Draugr writes one per repository and
...
The template collects `draugr-out/sbom-*.cdx.json` for you. Without `config.sbom` there is nothing
to collect, the runner says so, and the rest of the job is unaffected — so the tab reads
*"does not have license scanning reports"* until you enable it. Requires Syft, which
