...
If you are unsure between a scanner and a control: a **control** is a *question* (“are my
dependencies vulnerable?”), a **scanner** is one *way of answering it*. Two tools that answer the
same question are two scanners on one control, not two controls. That is what lets their findings
be ranked together by one risk model.
...
was ignored. Each produces a green tick that means nothing, and a green tick that means nothing is
worse than a red one, because nobody investigates it.
...
issued in thirty seconds. *non-commercial* is a common one, and it decides whether a user may run
the control at all.
