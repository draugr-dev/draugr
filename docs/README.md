...
Start here. The docs are grouped by section; every published page carries a `section` frontmatter
and is rendered by the docs site. This index and the `contributing/` pages have no `section` and are
not published. They live here for readers browsing the repo.
