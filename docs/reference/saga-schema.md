...
The publisher renders both documents a server needs, the `json` run report and the `sarif`
evidence, so `kind: draugr-api` is the whole of it. The token comes from `$DRAUGR_API_TOKEN` (or
