# Scanner: `trivy-license` (dependency licences)

- **Control:** [`licenses`](../controllers/licenses.md)
- **Tool:** Aqua **Trivy** (filesystem mode, licence scanner) — https://trivy.dev
...
SARIF output contains no licence findings at all — they live only under `Results[].Licenses[]` in
...
Rule ids are `license/<spdx>/<package>`. Licence first, so the common exemption —
*accept this licence anywhere* — is `license/MPL-2.0/*`, while the full id stays available for
...
findings. Without help, every licence in a project lands at the top of `go.mod` in a pile — the
...
- Trivy licence scanning: https://trivy.dev/latest/docs/scanner/license/
