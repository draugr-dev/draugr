...
var (
	...
	runCosign      = func(ctx context.Context, cosignPath string, args []string) error {
		out, err := exec.CommandContext(ctx, cosignPath, args...).CombinedOutput() // #nosec G204 -- cosignPath from LookPath; args built here // nosem: go.lang.security.audit.dangerous-exec-command.dangerous-exec-command -- verifying with the cosign CLI is the point; path from LookPath, args built here
		if err != nil {
