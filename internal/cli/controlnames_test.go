...
// namespacedComponent is a component claiming part of a cluster, which is the half of the
// pairing the descriptor writes on the component.
func namespacedComponent() saga.Component {
	return saga.Component{
		Name: "platform",
		Infrastructure: []saga.Infrastructure{
			{Kind: "kubernetes", Ref: "prod", Namespaces: []string{"platform-system"}},
		},
	}
}

// A component that claims three namespaces and a scanner that always reports eighty cannot both
// be honoured, and the scanner refuses when it is handed the target — correctly, because the
// alternative is filing somebody else's cluster against this component. But that happens partway
// through a run, after a cluster has been contacted, and it surfaces as a control that errored
// rather than as a descriptor that cannot work. Both halves are in the file, so validate answers
// it: no cluster, no credentials, no wait.
func TestCheckControlNamesRejectsAClusterWideScannerUnderANamespaceScope(t *testing.T) {
	m := &saga.Model{
		Config: saga.Config{Controllers: map[string]saga.ControllerSettings{
			"infrastructure": {"kubeBenchJob": saga.ControllerSettings{"enabled": true}},
		}},
		Components: []saga.Component{namespacedComponent()},
	}
	err := checkControlNames(builtins.Registry(), m)
	if err == nil {
		t.Fatal("a namespace scope handed to a whole-cluster scanner was accepted")
	}
	// The component, the scanner, and both ways out. An error naming only what is wrong leaves
	// the reader to work out which of the two declarations to change.
	for _, want := range []string{`"platform"`, "kube-bench-job", "remove `namespaces`", "kubeBenchJob.enabled: false"} {
		if !strings.Contains(err.Error(), want) {
			t.Errorf("error should mention %q: %v", want, err)
		}
	}
	// The closing line about `draugr controls` answers a question this reader does not have, and
	// competes with the instruction they do need.
	if strings.Contains(err.Error(), "draugr controls") {
		t.Errorf("a scope pairing needs no list of controls: %v", err)
	}
}

// The pairing is what fails, so neither half alone may.
func TestCheckControlNamesAcceptsEitherHalfOfTheScopePairingAlone(t *testing.T) {
	clusterWide := map[string]saga.ControllerSettings{
		"infrastructure": {"kubeBenchJob": saga.ControllerSettings{"enabled": true}},
	}
	wholeCluster := saga.Component{
		Name:           "platform",
		Infrastructure: []saga.Infrastructure{{Kind: "kubernetes", Ref: "prod"}},
	}
	for name, m := range map[string]*saga.Model{
		"namespaces, but the scanner is not enabled": {
			Config: saga.Config{Controllers: map[string]saga.ControllerSettings{
				// A block carrying config for a scanner nobody turned on. Every cluster-wide
				// scanner is a non-default one, so this never selects it.
				"infrastructure": {"kubeBenchJob": saga.ControllerSettings{"timeout": "5m"}},
			}},
			Components: []saga.Component{namespacedComponent()},
		},
		"the scanner is enabled, but nothing is narrowed": {
			Config:     saga.Config{Controllers: clusterWide},
			Components: []saga.Component{wholeCluster},
		},
		"the component turns the scanner off for itself": {
			Config: saga.Config{Controllers: clusterWide},
			Components: []saga.Component{{
				Name:           "platform",
				Infrastructure: namespacedComponent().Infrastructure,
				Controllers: map[string]saga.ControllerSettings{
					"infrastructure": {"kubeBenchJob": saga.ControllerSettings{"enabled": false}},
				},
			}},
		},
	} {
		if err := checkControlNames(builtins.Registry(), m); err != nil {
			t.Errorf("%s: rejected a descriptor that works: %v", name, err)
		}
	}
}

// One cluster is the case that hides this bug. `kubeBenchJob` is enabled once, for the project,
// and only one component narrows itself — so a check that answered per descriptor rather than per
// component would condemn the component that is entitled to the whole cluster, and a descriptor
// with a single component would never show it.
func TestCheckControlNamesBlamesOnlyTheComponentThatNarrowedItself(t *testing.T) {
	m := &saga.Model{
		Config: saga.Config{Controllers: map[string]saga.ControllerSettings{
			"infrastructure": {"kubeBenchJob": saga.ControllerSettings{"enabled": true}},
		}},
		Components: []saga.Component{
			{Name: "whole-cluster", Infrastructure: []saga.Infrastructure{{Kind: "kubernetes", Ref: "prod"}}},
			namespacedComponent(),
		},
	}
	err := checkControlNames(builtins.Registry(), m)
	if err == nil {
		t.Fatal("the narrowed component was accepted")
	}
	if !strings.Contains(err.Error(), `"platform"`) {
		t.Errorf("error should name the component that narrowed itself: %v", err)
	}
	if strings.Contains(err.Error(), "whole-cluster") {
		t.Errorf("a component claiming the whole cluster is served fine and must not be blamed: %v", err)
	}
}
