...
func requireNamespace(ctx context.Context, cs kubernetes.Interface, name string) error {
	...
	case apierrors.IsForbidden(err):
		// A credential scoped to a namespace often cannot read the namespace object itself. That is a
		// normal way to be configured, not a mistake, so it must not fail the survey. The listing that
		// follows will report its own permission error if there is one.
		return nil
	default:
