...
Plans one native header scan per host declared on a component (skipping hosts without a `url`), then
aggregates + deduplicates the findings into a per-control result with a severity summary. The
scanner tunes its checklist by each host's `type` (`browser`, the default, vs. `api`), so
browser-only headers aren't flagged on programmatic endpoints.
